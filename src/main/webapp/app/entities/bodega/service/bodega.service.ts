...
export class BodegaService {
  ...
  addBodegaToCollectionIfMissing<Type extends Pick<IBodega, 'id'>>(
    ...
    if (bodegas.length > 0) {
      const bodegaCollectionIdentifiers = bodegaCollection.map(bodegaItem => this.getBodegaIdentifier(bodegaItem));
      const bodegasToAdd = bodegas.filter(bodegaItem => {
        const bodegaIdentifier = this.getBodegaIdentifier(bodegaItem);
        if (bodegaCollectionIdentifiers.includes(bodegaIdentifier)) {
          return false;
        }
